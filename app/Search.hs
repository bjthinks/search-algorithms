...
breadthFirstTreeSearch expandState startState isGoal
  | isGoal startState = (Just [startState],SearchStats 0 0)
  | otherwise = bfts [[startState]] [] (SearchStats 0 0)
  where
    bfts [] [] stats = (Nothing,stats)
    bfts [] new_frontier stats = bfts new_frontier [] stats
    bfts (this_path:rest_of_frontier) new_frontier (SearchStats n s) =
      ...
      in if null goals_from_here
         ...
         else (Just $ head goals_from_here : this_path,SearchStats (n+1) s)

breadthFirstGraphSearch :: Ord state => (state -> [state]) -> state ->
                           (state -> Bool) -> (Maybe [state],SearchStats)
breadthFirstGraphSearch expandState startState isGoal
  | isGoal startState = (Just [startState],SearchStats 0 0)
  | otherwise = bfts [[startState]] [] (S.singleton startState) (SearchStats 0 0)
  where
    bfts [] [] _ stats = (Nothing,stats)
    bfts [] new_frontier explored_set stats
      = bfts new_frontier [] explored_set stats
    bfts (this_path:rest_of_frontier) new_frontier explored_set (SearchStats n s) =
      ...
      in if null goals_from_here
         then bfts rest_of_frontier
              ...
              (SearchStats (n+1) (s+length from_here))
         else (Just $ head goals_from_here : this_path,SearchStats (n+1) s)
...
uniformCostTreeSearch expandState startState isGoal =
  ucts (H.singleton (0,[startState]) :: H.MinPrioHeap cost [state]) (SearchStats 0 0)
  where
    ucts h stats@(SearchStats n s) = case H.view h of
...
uniformCostGraphSearch expandState startState isGoal =
  ...
  where
    ucgs h explored_set stats@(SearchStats n s) = case H.view h of
      Nothing -> (Nothing,stats)
      Just ((current_cost,current_path),h') ->
        let current_state = head current_path
        ...
        in if isGoal current_state
           then (Just (current_path,current_cost),stats)
                ...
                then ucgs h' explored_set (SearchStats n (s-1))
